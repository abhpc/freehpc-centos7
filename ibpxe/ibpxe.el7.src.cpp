...
//const string IMG = "initrd-pxe.3.10.0-957.el7.x86_64.img";
// ip addr show ib0|grep -i link|awk '{print $2}'
...
int main(int argc, char* argv[]) {
    ...
    const string dhcp_conf_path = CONF_DIR + "/dhcpd.conf";

    struct stat file_stat;
    if (stat(dhcp_conf_path.c_str(), &file_stat) != 0) {
        std::cerr << "Error: dhcpd.conf not found in specified directory." << std::endl;
        return -1;
    }
    ...
    if (stat(image_root_init.c_str(), &init_stat) != 0 || stat(node_root_init.c_str(), &init_stat) != 0) {
        std::cerr << "Error: init scripts not found." << std::endl;
        return -1;
    }

    //system("dos2unix imag_root/init.src &>/dev/null");
    //system("dos2unix node_root/init.src &>/dev/null");
    ...
    cmd = "\\cp -f "+ CONF_DIR +"/dhcpd.conf /etc/dhcp/dhcpd.conf";
    system(cmd.c_str());
    cmd = "echo \"DHCPDARGS=\\\"" + Master_IB + "\\\"\" > /etc/sysconfig/dhcpd";
    system(cmd.c_str());
    system("service dhcpd restart");
