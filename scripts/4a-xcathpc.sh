...
MST_IP=$(ip addr show bond0|grep "inet "|awk -F "/" '{print $1}'|awk '{print $2}')
...
say **********************************************
say Welcome to XCATHPC.
say XCAT Diskless High Performance Clusters.
say http://www.xcathpc.com
say **********************************************

ALLOWOPTIONS 1

MENU TITLE XCATHPC (http://www.xcathpc.com)

label abhpc
  MENU DEFAULT
  MENU LABEL XCATHPC OS on CentOS 7.9
  IPAPPEND 1
  kernel vmlinuz-pxe
  ...
  TEXT HELP
  * XCATHPC version: 2024b (C) 2016-2034
  * Disclaimer: XCATHPC is a HPC OS based on open source code XCAT
  ENDTEXT
EOF
...
image=$(ls /tftpboot/nbi_img/initrd-pxe.*.img |awk -F "/" '{print $4}')
